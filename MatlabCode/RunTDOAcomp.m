...
figure;
simThreshs = [ .85];%linspace(.95,.998, 3);
TimeThreshs = [10 15 20];

 simThreshs = [.98];%linspace(.95,.998, 3);
% TimeThreshs = [30 ];
%%%%%%%%%%%%%%%%%% FUCING AROUND
...
for ii =1:length(simThreshs)
    ...
    for jj =1:length(TimeThreshs)
        ...
        simStructTDOA.chains = updateChainsEncounterBESTFirstKAC(simStructTDOA);
